...
# code2llm | 265f 14882L | proto:138,python:75,yaml:31,shell:15,json:2,yml:1,toml:1 | 2026-06-07
# generated in 0.07s
# CC̅=3.3 | critical:1/404 | dups:0 | cycles:0
...
PIPELINES[172]:
  ...
  [29] Src [_parse_open_prefix]: _parse_open_prefix → _normalize_app_name
      PURITY: 100% pure
  [30] Src [desktop_id_for_app]: desktop_id_for_app → desktop_id_candidate_names
      PURITY: 100% pure
  [31] Src [_legacy_nlp2uri_settings]: _legacy_nlp2uri_settings → _compile_settings → _linux_settings_actions → _first_available
      PURITY: 100% pure
  [32] Src [_legacy_nlp2uri_app_open]: _legacy_nlp2uri_app_open → _compile_launch_app → _linux_launch_app_actions → _first_available
      PURITY: 100% pure
  [33] Src [_legacy_nlp2uri_app_focus]: _legacy_nlp2uri_app_focus → _compile_window → _compile_window_focus → _linux_window_focus_actions → ...(1 more)
      PURITY: 100% pure
  [34] Src [with_params]: with_params
      PURITY: 100% pure
  [35] Src [to_slots]: to_slots
      PURITY: 100% pure
  ...
  [39] Src [to_dict]: to_dict
      PURITY: 100% pure
  [40] Src [default]: default → ensure_config → find_config_path → config_search_paths
      PURITY: 100% pure
  [41] Src [for_platform]: for_platform → load_config → find_config_path → config_search_paths
      PURITY: 100% pure
  [42] Src [_cfg]: _cfg → load_config → find_config_path → config_search_paths
      PURITY: 100% pure
  [43] Src [_host]: _host → get_effective_platform → load_config → find_config_path → ...(1 more)
      PURITY: 100% pure
  [44] Src [from_prompt]: from_prompt → nlp2uri → get_effective_platform → load_config → ...(2 more)
      PURITY: 100% pure
  [45] Src [resolve]: resolve → resolve_text → parse_text → _normalize_aliases
      PURITY: 100% pure
  [46] Src [compile]: compile → compile_uri_to_actions → _query_params
      PURITY: 100% pure
  [47] Src [execute]: execute → execute_uri → get_effective_platform → load_config → ...(2 more)
      PURITY: 100% pure
  [48] Src [handle_prompt]: handle_prompt
      PURITY: 100% pure
  [49] Src [handle_uri]: handle_uri
      PURITY: 100% pure
  [50] Src [list_system_uris]: list_system_uris → build_uri_index → _index_runtimes → _ir_field
      PURITY: 100% pure

LAYERS:
  src/                            CC̄=3.3    ←in:0  →out:0
  │ !! compile                    577L  0C   50m  CC=13     ←5
  │ mcp                        382L  1C   18m  CC=6      ←0
  │ parse_nl                   376L  0C   21m  CC=6      ←1
  │ index                      351L  2C   23m  CC=7      ←5
...
# redup/duplication | 12 groups | 75f 7289L | 2026-06-07
...
HOTSPOTS[7] (files with most duplication):
  src/nlp2uri/systemmap/index.py  dup=70L  groups=3  frags=7  (1.0%)
  src/nlp2uri/schemes/desktop.py  dup=36L  groups=1  frags=2  (0.5%)
  src/nlp2uri/compile.py  dup=26L  groups=2  frags=4  (0.4%)
...
DUPLICATES[12] (ranked by impact):
  ...
  [85ffa2defd9f0fa1]   STRU  _windows_window_move_actions  L=10 N=2 saved=10 sim=1.00
      src/nlp2uri/compile.py:440-449  (_windows_window_move_actions)
      src/nlp2uri/compile.py:486-491  (_windows_window_focus_actions)
  ...
  [a5e2055bcecafecc]   STRU  _macos_window_move_actions  L=7 N=2 saved=7 sim=1.00
      src/nlp2uri/compile.py:431-437  (_macos_window_move_actions)
      src/nlp2uri/compile.py:481-483  (_macos_window_focus_actions)
...
REFACTOR[12] (ranked by priority):
  ...
  [2] ○ extract_function   → src/nlp2uri/schemes/utils/build_focus.py
  ...
  [4] ○ extract_function   → src/nlp2uri/utils/_parse_file_open.py
  ...
  [7] ○ extract_function   → src/nlp2uri/systemmap/utils/to_dict.py
      WHY: 2 occurrences of 8-line block across 2 files — saves 8 lines
      FILES: src/nlp2uri/systemmap/getv_uri.py, src/nlp2uri/systemmap/resolve.py
  [8] ○ extract_function   → src/nlp2uri/utils/_macos_window_move_actions.py
      WHY: 2 occurrences of 7-line block across 1 files — saves 7 lines
      FILES: src/nlp2uri/compile.py
  [9] ○ extract_function   → src/nlp2uri/cqrs/drivers/utils/probe.py
      WHY: 2 occurrences of 6-line block across 2 files — saves 6 lines
      FILES: src/nlp2uri/cqrs/drivers/resource_probe.py, src/nlp2uri/cqrs/drivers/runtime_curl.py
  [10] ○ extract_function   → src/nlp2uri/systemmap/utils/_ir_field.py
      WHY: 2 occurrences of 4-line block across 2 files — saves 4 lines
      FILES: src/nlp2uri/systemmap/export.py, src/nlp2uri/systemmap/index.py
  [11] ○ extract_function   → src/nlp2uri/systemmap/utils/uri_for_schedule.py
      WHY: 2 occurrences of 4-line block across 1 files — saves 4 lines
      FILES: src/nlp2uri/systemmap/uri.py
  [12] ○ extract_function   → src/nlp2uri/systemmap/utils/uri_for_conversation.py
...
QUICK_WINS[9] (low risk, high savings — do first):
  [1] extract_function   saved=33L  → src/nlp2uri/systemmap/utils/_index_resources.py
      FILES: index.py
  [2] extract_function   saved=18L  → src/nlp2uri/schemes/utils/build_focus.py
      FILES: desktop.py
  [3] extract_function   saved=12L  → src/nlp2uri/systemmap/utils/_compile_resource.py
      FILES: compile.py
  [4] extract_function   saved=11L  → src/nlp2uri/utils/_parse_file_open.py
      FILES: parse_nl.py
  ...
  [6] extract_function   saved=10L  → src/nlp2uri/utils/_windows_window_move_actions.py
      FILES: compile.py
  [7] extract_function   saved=8L  → src/nlp2uri/systemmap/utils/to_dict.py
      FILES: getv_uri.py, resolve.py
  [8] extract_function   saved=7L  → src/nlp2uri/utils/_macos_window_move_actions.py
      FILES: compile.py
  [9] extract_function   saved=6L  → src/nlp2uri/cqrs/drivers/utils/probe.py
...
# code2llm/evolution | 397 func | 64f | 2026-06-07
# generated in 0.01s

NEXT[4] (ranked by impact):
  [1] !! SPLIT           src/nlp2uri/compile.py
      WHY: 577L, 0 classes, max CC=13
      EFFORT: ~4h  IMPACT: 7501

  [2] !  SPLIT-FUNC      _match_command_entry  CC=16  fan=13
...
RISKS[3]:
  ...
  ⚠ Splitting src/nlp2uri/compile.py may break 50 import paths
